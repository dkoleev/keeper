...
namespace Avocado.Editor.LevelCreator {
    public static class LevelCreatorHelper {
        ...
        public static void DrawBrush(LevelCreatorPreferences preference, SceneObjectBrush brush, Vector3 pos, float radius, Vector3 normal){
        ...
        /*public static void SetOffsetSceneObject(GameObject so, string key) {
            ...
            var isFound = false;
            foreach (var soData in DB.instance.GameData.GetSceneObjects()) {
                if (soData.Id == key) {
                    size = new Point(soData.Size.Width, soData.Size.Height);
                    isFound = true;
                    break;
                }
            }
            ...
            if (!isFound) {
                ...
                var offset = so.transform.Find(SceneObjectView.OffsetName);
                offset.localPosition = SceneObjectView.CalculateOffsetPosition(size);
            }
        }*/
    }
}
